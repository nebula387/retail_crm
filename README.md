...
### Шаг 1: Проект
Создана папка проекта в пк, виртуальное окружение, клонирование git в VsCode для работы локально с Claude Code

### Шаг 2: Аккаунты
Созданы все необходимые аккаунты: RetailCRM (демо), Supabase (free tier), Vercel (Hobby), Telegram Bot через @BotFather.

### Шаг 3: Знакомство с RetailCRM

### Шаг 4: Начало работы, промпт проекта 
```
я зарегистрировался в RetailCRM, Supabase, Vercel создал телеграм бота, нужно выполнить проект в папке project_crm, напиши програму и мои дальнейшие шаги
```
Далее выполнение проекта по шагам, connect github и деплой на vercel 

### Шаг 5: Загрузка заказов
Скрипт `scripts/upload_to_retailcrm.py` загрузил 50 заказов из `mock_orders.json` в RetailCRM через `/api/v5/orders/create`.

### Шаг 6: RetailCRM → Supabase
...
### 3. Vercel не применял новые env vars
...
### 4. RetailCRM не поддерживает вебхуки в демо-аккаунте
...
### 5. Дублирующиеся Telegram-уведомления
